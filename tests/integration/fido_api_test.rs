...
async fn test_attestation_options_success() {
    ...
    assert!(result["user"]["id"].as_str().is_some());
    assert!(result["user"]["name"].as_str().is_some());
    assert!(result["user"]["displayName"].as_str().is_some());
...
#[actix_web::test]
async fn test_attestation_result_invalid_empty_id() {
    let webauthn_config = WebAuthnConfig::default();
    let webauthn_service: Arc<dyn WebAuthnService> = Arc::new(WebAuthnServiceImpl::new(webauthn_config));
...
async fn test_assertion_options_success() {
    ...
    assert!(result["userVerification"].as_str().is_some());
}
