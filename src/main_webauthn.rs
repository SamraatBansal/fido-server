...
use fido2_webauthn_server::webauthn_service::WebAuthnService;
use fido2_webauthn_server::webauthn_handlers::*;
...
async fn main() -> std::io::Result<()> {
    ...
    // Initialize WebAuthn service with proper webauthn-rs implementation
    ...
    let webauthn_service = match WebAuthnService::new(&rp_id, &rp_name, &rp_origin) {
        Ok(service) => service,
        Err(e) => {
    ...
    tracing::info!("Using WebAuthn-rs with in-memory storage");
