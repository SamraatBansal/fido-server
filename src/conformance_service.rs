...
impl ConformanceWebAuthnService {
    ...
    fn validate_fido_u2f_attestation_statement(&self, att_stmt: &std::collections::BTreeMap<serde_cbor::Value, serde_cbor::Value>) -> Result<()> {
        // FIDO U2F attestation statement validation
        let mut has_sig = false;
        let mut has_x5c = false;
            ...
            if let serde_cbor::Value::Text(key_str) = key {
                match key_str.as_str() {
                    "sig" => {
                        has_sig = true;
                        if !matches!(value, serde_cbor::Value::Bytes(_)) {
                            return Err(AppError::InvalidField("fido-u2f attStmt.sig must be bytes".to_string()));
                        }
                        if let serde_cbor::Value::Bytes(sig_bytes) = value {
                            if sig_bytes.is_empty() {
                                return Err(AppError::InvalidField("fido-u2f attStmt.sig cannot be empty".to_string()));
                            }
                        }
                    },
                    "x5c" => {
                        has_x5c = true;
                        if !matches!(value, serde_cbor::Value::Array(_)) {
                            return Err(AppError::InvalidField("fido-u2f attStmt.x5c must be an array".to_string()));
                        }
                        if let serde_cbor::Value::Array(x5c_array) = value {
                            if x5c_array.len() != 1 {
                                return Err(AppError::InvalidField("fido-u2f attStmt.x5c must contain exactly one certificate".to_string()));
                            }
                        }
                    },
                    _ => {} // Ignore unknown fields
        ...
        if !has_sig {
            return Err(AppError::MissingField("fido-u2f attStmt.sig".to_string()));
        }
        if !has_x5c {
            return Err(AppError::MissingField("fido-u2f attStmt.x5c".to_string()));
        }
